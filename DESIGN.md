...
- **Tertiary (Navy Blue):** Used for headers, footers, and authoritative typography to ground the design in stability and trust.
...
- **Buttons:** Use `rounded-lg` (1rem) for a friendly, modern feel.
- **Cards:** Use `rounded-lg` (1rem) with overflow-hidden to clip imagery cleanly.
...
- **Impact Counters:** Large-scale numbers in Deep Green (#138808) to represent transparency and results. Accompanied by a label in Navy Blue.
...
- **Input Fields:** Minimalist design with a 1px Navy Blue bottom border that transitions to a Saffron focus state.